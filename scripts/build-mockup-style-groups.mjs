...
function writeTable(table, views, perVariant) {
...
export const PER_VARIANT_MOCKUP_PRODUCTS = ${JSON.stringify(perVariant ?? [], null, 2)};
`;
  ...
  writeFileSync(
    ...
    `${header}\n${body.replace('export const MOCKUP_STYLE_GROUPS =', 'export const MOCKUP_STYLE_GROUPS: Record<string, Record<string, string>> =')
      ...
      .replace('export const PER_VARIANT_MOCKUP_PRODUCTS =', 'export const PER_VARIANT_MOCKUP_PRODUCTS: string[] =')}`
  );
}

// Which products Printful photographs PER VARIANT, i.e. those with any `restricted_to_variants`
// style. Two different consequences hang off this one fact, and only the first was being used:
//
//   - The table build must walk every variant of such a product, because one variant's ids do not
//     classify another's (that is `styleCoverage`'s own `restricted`, below).
//   - A PREVIEW of one variant is not a preview of another. useMockup otherwise keys a cached
//     mockup on the variant's COLOUR, on the reasoning that sizes share printfile ids and Printful
//     photographs one garment for all of them. True for a garment; false here.
//
// Always swept over the WHOLE catalogue, never the --products= subset, so the emitted set is
// complete however the builder was invoked and there is nothing to merge. It costs one free catalog
// read per product and not a single mockup task, which is what lets --regenerate produce it too.
async function perVariantMockupProducts() {
  const found = [];
  for (const productId of Object.keys(PRODUCT_MOCKUP_CONFIG)) {
    const { body } = await pf(`/v2/catalog-products/${productId}/mockup-styles`);
    const restricted = (body?.data ?? []).some(entry =>
      (entry?.mockup_styles ?? []).some(st => st?.restricted_to_variants)
    );
    if (restricted) found.push(productId);
  }
  return found;
}

// Re-emit both files from the table already in the repo. Makes no MOCKUP TASKS -- it does read each
// product's style list, which is free, to refresh PER_VARIANT_MOCKUP_PRODUCTS. Use after changing
// which groups count as on-model; use the full run when a PRODUCT's styles change.
if (args.includes('--regenerate')) {
  ...
  const perVariant = await perVariantMockupProducts();
  writeTable(JSON.parse(match[1]), viewsMatch ? JSON.parse(viewsMatch[1]) : {}, perVariant);
  console.log(`Regenerated both files from the existing table. Per-variant products: ${perVariant.join(', ') || 'none'}.`);
  process.exit(0);
}
...
if (args.includes('--names')) {
  ...
  writeTable(table, views, await perVariantMockupProducts());
...
writeTable(table, existingViews, await perVariantMockupProducts());
