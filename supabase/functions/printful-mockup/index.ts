...
// generator_mockup_id -> style group, and the on-model group chosen per product. GENERATED by
// scripts/build-mockup-style-groups.mjs, which writes this file and its src/lib twin together so the
// two cannot drift -- same discipline as _shared/compactDesign.ts mirroring render/compactDesign.js.
// It is what lets ONE task carry flats, detail shots and on-model shots: v1 leaves a placement's
// primary photo untyped, and this is the only thing that says which group that photo came from.
import { MOCKUP_STYLE_GROUPS, MODEL_GROUPS_BY_PRODUCT, MOCKUP_STYLE_VIEWS } from "../_shared/mockupStyleGroups.ts";
...
Deno.serve(async req => {
  ...
  if (req.method === "GET") {
    ...
    // Optional, and only used to look up the style table -- polling is by task_key alone, so a
    // caller that omits it (or an id the table does not know) simply gets untyped photos, which the
    // client then ranks last rather than guessing at. Never used to fetch anything.
    const productId = url.searchParams.get("productId") ?? "";
    const styleGroups: Record<string, string> = MOCKUP_STYLE_GROUPS[productId] ?? {};
    const modelGroup = MODEL_GROUPS_BY_PRODUCT[productId] ?? null;
    const styleViews: Record<string, string> = MOCKUP_STYLE_VIEWS[productId] ?? {};
    ...
    const byUrl = new Map<
      string,
      { mockup_url: string; display_name: string; option_group: string | null; generator_mockup_id: number | null }
    >();
    ...
    const raw: Array<{ url: string; name: string; optionGroup?: string; id?: number }> = [];
    const seenUrls = new Set<string>();
    const seenViews = new Set<string>();
    const push = (url: string | undefined, name: string, optionGroup?: string, id?: number) => {
      if (!url || seenUrls.has(url)) return;
      // ALSO de-duplicate by identity, not URL alone. v1 repeats a product's camera angles under
      ...
      // the strip with "Back 2, Back 3, Back 4, Back 5".
      //
      // `generator_mockup_id` is that identity, and it replaced a group+title key when the on-model
      // tier landed. The old key could not tell a flat back from an on-model back: same title, and
      // both untyped, so one of the two was silently dropped -- exactly the photo the model tier
      // exists to add. The id is per style-view, so its repeats across placements are genuinely the
      // same photo. Falls back to the old key for an entry carrying no id.
    ...
    // Extras keep Printful's OWN title, which is authoritative for them -- unlike a primary, an
    // extra states the view it shows. Only the primaries need the table.
    ...
    for (const m of ordered) {
      ...
      // labelled "Outside label".
      //
      // So the placement may not NAME it -- but dropping it outright, which is what this did until
      // the style table existed, throws the photo away. That cost the sweatshirt and the joggers
      // their Product details shot the moment the on-model tier was added: those photos had been
      // riding in on a sleeve placement, the model shots took those placements, and the only
      // remaining copy sat under a label placement. With a name from the table there is nothing
      // wrong with the photo itself, so it is kept and named honestly; with no table entry there is
      // still nothing safe to call it, and it is skipped exactly as before.
      ...
      // A primary is named from the style table wherever the table knows it, and from the placement
      // key only as a fallback. THE PLACEMENT KEY IS NOT A STATEMENT ABOUT WHAT THE PHOTO SHOWS:
      // on the track jacket the `front` placement returns the garment's back and vice versa (which
      // is why a hand-written per-product name swap existed here for a year), and the same fault
      // silently hit the joggers and the crossbody, where two different flats both arrived on a
      // placement labelled "Front" and one became "Front 2". The table's name comes off Printful's
      // own FILENAME, which is honest in every case measured -- including the ones where the
      // placement is wrong, which is what makes it a fix rather than a second guess.
      push(m.mockup_url, tableName ?? PLACEMENT_LABELS[m.placement] ?? m.placement, undefined, m.generator_mockup_id);
    }
    // Resolve each photo's group before naming, because the name depends on it. An untyped primary
    // is no longer necessarily a flat lay -- with the on-model tier requested it is just as likely
    // to be a model shot -- so the style table is what separates them.
    const resolved = raw.map(r => ({
      ...r,
      group: r.optionGroup ?? (r.id ? styleGroups[String(r.id)] ?? null : null)
    }));
    // On-model views say so in their label. Without this the model front and the flat front are
    // both "Front" and one becomes "Front 2", which tells the customer nothing about the difference
    // between two photos that look quite unalike. The model group is named per product by the
    // generated table rather than pattern-matched here, so the tier definition stays in one place
    // (src/lib/printfulViewPolicy.js) instead of being restated in Deno.
    //
    // It is a SUFFIX, not a prefix, and that is load-bearing rather than a wording preference:
    // viewRank orders a strip by matching the START of a title (/^front/, /^back/, ...), so "On
    // model - Front" would rank as unrecognised and the model shots would come out in whatever
    // order Printful happened to return them. "Front on model" still sorts as a front.
    // An on-model view is named from the style table, which took the name off Printful's own
    // FILENAME, because the placement key a primary arrives under is not a reliable statement about
    // what the photo shows: on the joggers the on-model FRONT arrives under the `back` placement, so
    // naming it from the placement produced "Back on model" on a photo of the front -- and that
    // wrong name then sorted it wrongly too, since viewRank reads the title. Primaries are named
    // from the table everywhere now, not just here, so this step only has the suffix left to add.
    ...
    for (const { url, name, group, id } of named) {
      ...
      byUrl.set(url, {
        mockup_url: url,
        display_name: final,
        option_group: group ?? null,
        // Passed through so the client can rank a photo the table here could not name -- and so a
        // frontend newer than this function is never left with nothing to classify by.
        generator_mockup_id: id ?? null
      });
    }
    return Response.json(
      {
